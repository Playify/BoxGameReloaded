...
class FontRenderer {
    ...
    public String trimStringToWidth(String s, float width) {
        int i = 0;
        for (; i < s.length(); i++) {
            width -= charWidth[this.str.indexOf(s.charAt(i))];
            if (width <= 0) {
                i--;
                break;
            }
    ...
    void drawMultiline(String s, float x, float y, float w) {
        ...
        for (int ii = 0; ii < chars.length; ii++) {
            ...
            if (chars[ii] == '\0') {
                ...
                glBindTexture(GL_TEXTURE_2D, 0);
                glDisable(GL_TEXTURE_2D);
                d.rect(offset - 1 / 16f, 0, charWidth[str.indexOf(chars[ii + 1])] / 8f + (chars.length < ii + 2 ? chars[ii + 2] == '\r' ? .1f : 0 : 0), 1, 0x66000000);
                glEnable(GL_TEXTURE_2D);
                glBindTexture(GL_TEXTURE_2D, textureID);
                glEnableClientState(GL_TEXTURE_COORD_ARRAY);
            } else if (i != -1) {
